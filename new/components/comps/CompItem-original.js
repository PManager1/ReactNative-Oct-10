...
class CompItem extends Component {
  ...
  renderDescription(){
    ...
    if (expanded) {
        return(
        <Text style={styles.renderDescriptionTextStyle}> {comp.address[0].street[0]} {comp.address[0].city[0]} {comp.address[0].state[0]} | {comp.bedrooms[0]} BD  {comp.bathrooms[0]} BA |    |  L Sold {comp.lastSoldDate[0]} for ${comp.lastSoldPrice[0]._}  which is  {comp.finishedSqFt[0]} SqFt, | lotSize = {comp.lotSizeSqFt[0]} | yearBuilt = {comp.yearBuilt[0]}  </Text>
